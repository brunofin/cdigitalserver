...
public class CtrItemListar implements Controle {
	...
	private void atualizarTabela() {
		...
		try {
			...
			System.out.println("<CtrItemListar> Erro ao listar Itens: "	+ e.getMessage());
		}
	...
	private void adicionarListeners() {
		...
		form.getBtnVerEditar().addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (form.getTabelaItens().getSelectedRow() >= 0) {
					Item itemParaEditar = listaItens.get(form.getTabelaItens()
							.getSelectedRow());
		...
		form.getBtnExcluir().addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (form.getTabelaItens().getSelectedRow() >= 0) {
					Item itemParaExcluir = listaItens.get(form.getTabelaItens()
							.getSelectedRow());
					DAOFactory factory = DAOFactory
							.getDaoFactory(Database.MYSQL);
					itemDAO = factory.getItemDAO();
					boolean excluiu = false;
					try {
						excluiu = itemDAO.exluir(itemParaExcluir);
					} catch (SQLException e1) {
						System.out.println("Erro ao exlcuir item" + e1.getMessage());
